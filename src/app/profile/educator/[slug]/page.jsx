...
// Deferred — ViewProfile is 1900+ lines with heavy deps (Vimeo, framer-motion, etc.)
// Splitting it here keeps the educator profile initial bundle lean.
const ViewProfile = dynamic(
  () => import("@/components/Educator/ViewProfile"),
  { loading: () => <ViewProfileLoader /> }
);
import { getEducatorBySlug, getEducatorProfile } from "@/components/server/educators.routes";
...
const Page = ({ params }) => {
  ...
  useEffect(() => {
    if (!resolvedParams?.slug) {
      setError("No educator slug provided");
      setLoading(false);
      return;
    }
    ...
    const fetchEducators = async () => {
      try {
        const isObjectId = /^[a-f\d]{24}$/i.test(resolvedParams.slug);
        const apiResponse = isObjectId
          ? await getEducatorProfile(resolvedParams.slug)
          : await getEducatorBySlug(resolvedParams.slug);

...
const CoursesSection = ({ courses }) => {
  return (
    <div className="mb-8">
      <h2 className="text-xl font-semibold text-gray-800 mb-6">Courses by this Educator</h2>
      ...
      {courses.length === 0 && (
        <div className="text-center py-10 bg-gray-50 rounded-lg">
          <p className="text-gray-500">No courses available from this educator yet.</p>
        </div>
      )}
    </div>
  );
...
const CourseCard = ({ course }) => {
  const { id, title, thumbnail, price, originalPrice, duration, students, rating } = course;

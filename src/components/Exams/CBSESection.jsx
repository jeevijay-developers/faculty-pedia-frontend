  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        setError('');
        const data = await fetchIITJEEOnlineCourses('CBSE', {
          limit: 12,
          sortBy: 'startDate',
          sortOrder: 'asc',
          status: 'ongoing',
        ...
        console.error('Failed to load CBSE courses', err);
        if (isMounted) {
          setError('Unable to load CBSE courses right now.');
          setCourses([]);
        }
      } finally {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          {loading ? (
            ...
            <div className="py-16 text-center text-gray-500 dark:text-gray-400">No CBSE courses available right now.</div>
          ) : (
            <Swiper
              modules={[Navigation]}

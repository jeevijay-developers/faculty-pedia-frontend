...
const AllTestSeries = ({ exam = 'IIT-JEE' }) => {
  ...
  const TestSeriesCard = ({ testSeries }) => (
    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-all duration-300 hover:border-blue-300 overflow-hidden">
      {/* Header Section */}
      <div className="p-3 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <span className="text-gray-900 dark:text-gray-100 font-medium">{testSeries.teacher.name}</span>
              </div>
              <div className="flex flex-wrap items-center mt-1">
                <div className="flex items-center space-x-1 mr-3">
                  <FaGraduationCap className="w-3 h-3 text-gray-500 dark:text-gray-400" />
                  <span className="text-xs text-gray-600 dark:text-gray-400">{testSeries.teacher.qualification}</span>
                </div>
                <span className="text-xs text-gray-600 dark:text-gray-400">{testSeries.teacher.experience}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 px-3 py-2 rounded-lg">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 text-right">Course Fee</div>
          </div>
        </div>
      </div>
      ...
      <div className="p-3">
        {/* Title */}
        <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 leading-tight capitalize mb-3">
          {testSeries.title}
        </h3>
        ...
        <div className="grid grid-cols-3 gap-4 mb-2">
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-center">
            ...
            <div className="text-sm font-bold text-gray-900 dark:text-gray-100">{testSeries.noOfTests} tests</div>
          </div>
          
          <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-3 text-center">
            <MdDateRange className="w-5 h-5 text-green-600 mx-auto mb-1" />
            <div className="text-xs text-gray-500 dark:text-gray-400">Starting Date</div>
            <div className="text-sm font-bold text-gray-900 dark:text-gray-100">{formatDate(testSeries.startDate).split(' ').slice(0, 2).join(' ')}</div>
          </div>
          ...
          <div className="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-3 text-center">
            ...
            <div className="text-sm font-bold text-gray-900 dark:text-gray-100">{calculateValidity(testSeries.startDate, testSeries.endDate)}</div>
          </div>
        </div>
        
        {/* Action Buttons */}
        <div className="flex items-center gap-3 float-end mb-3 pt-2 border-t border-gray-100 dark:border-gray-800">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {filteredTestSeries.length > 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No test series found</h3>
